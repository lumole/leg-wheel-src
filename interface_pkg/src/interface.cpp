...
Interface::Interface()
{
    ...
    cmb_output_sub = n.subscribe("/cmb_master", 5, &Interface::CMB_Control, this);
    // can minipcie
...
void Interface::VMC_Control(const message::vmc_interface::ConstPtr &msg)
{
    ...
    else
    {
        ...
        while (counter_resent < max_resent)
        {
            ...
            Cyber_Gear_Motor_Torque(0x00000001, (-1) * msg->T_r);
            Motor_Feedback();
            counter_resent++;
        }
        counter_resent = 0; // 右轮
        while (counter_resent < max_resent)
        {
            ...
            Cyber_Gear_Motor_Torque(0x00000002, (1) * msg->T_l);
            Motor_Feedback();
            counter_resent++;
        }
        counter_resent = 0; // 右后
...
void Interface::M_Control(const message::manipulator::ConstPtr &msg) // 瓴控电机-自搭机械臂函数
{
    int speed_limit = 300;
...
void Interface::Motor_Feedback() // 接收电机反馈消息并判断ID
{
    ...
    for (i = 0; i < recvlen; i++)
    {
        ...
        if (recv_msg[i].ID == 0x028001FD) // 小米电机接收部分
        {
            int a = static_cast<int>(recv_msg[i].Data[2] * 256 + recv_msg[i].Data[3]);
            // printf("vel1 = %d\n", a);
            motor_feedback_msg.speed1 = linearTransform(a, 30); // 是否需要有负号   //0--65535分别对应-30--30
        ...
        if (recv_msg[i].ID == 0x028002FD)
        {
            int a = static_cast<int>(recv_msg[i].Data[2] * 256 + recv_msg[i].Data[3]);
            // printf("vel2 = %d\n", vel);
            motor_feedback_msg.speed2 = linearTransform(a, 30);
...
void Interface::CMB_Control(const message::cmb_interface::ConstPtr &msg)
{
    if (msg->F_flag == 0)
    {
        ...
        ROS_ERROR("-------------[ CMB对接控制 ]-------------\n");
        printf("-------------[  位置 开环  ]-------------\n");
        // Cyber_Gear_Motor_Torque(0x00000001, (-1) * msg->T_r);  //轮毂电机还是输出扭矩
        Cyber_Gear_Motor_Speed(0x00000002, (-1) * msg->T_r); // 轮毂电机还是输出速度
        Motor_Feedback();
        // Cyber_Gear_Motor_Torque(0x00000002, (1) * msg->T_l);
        Cyber_Gear_Motor_Speed(0x00000001, (1) * msg->T_l);
        Motor_Feedback();
        // 20240918 后续可以改一下下面这些指令里的角速度，改成时变的
        Motor_Pos_Transmit(0x00000143, (msg->P1_r - 180) * 800, 200); // 右后    //关节电机输出关节的位置
        Motor_Feedback();
        Motor_Pos_Transmit(0x00000144, (180 - msg->P1_l) * 800, 200); // 左后
        Motor_Feedback();
        Motor_Pos_Transmit(0x00000145, msg->P2_r * 800, 200); // 右前
        Motor_Feedback();
        Motor_Pos_Transmit(0x00000146, -msg->P2_l * 800, 200); // 左前
        Motor_Feedback();
        motor_feedback_msg.header.stamp = ros::Time::now();
        motor_pub.publish(motor_feedback_msg);
    }
    else
    {
        ...
        motor_feedback_msg.call6 = false;
        bool isRobotUnsafe = false;
        if (msg->flag == 0 || temprature_err_flag) // 温度警报器
        {
            // Motor_F_Transmit(0x00000141, 0);
            Cyber_Gear_Motor_Speed(0x00000002, 0);
            Motor_Feedback();
            Cyber_Gear_Motor_Speed(0x00000001, 0);
            Motor_Feedback();
            Motor_F_Transmit(0x00000143, 0);
            Motor_Feedback();
            Motor_F_Transmit(0x00000144, 0);
        ...
        else
        {
            ...
            while (counter_resent < max_resent)
            {
                ...
                // Motor_F_Transmit(0x00000141, (-1) * msg->T_r);
                // Cyber_Gear_Motor_Torque(0x00000001, (-1) * msg->T_r);
                Cyber_Gear_Motor_Speed(0x00000001, (1) * msg->dx_l);
                // printf("v_l = %f\n",msg->dx_l);
                Motor_Feedback();
                counter_resent++;
            }
            ...
            while (counter_resent < max_resent)
            {
                ...
                // Motor_F_Transmit(0x00000142, (1) * msg->T_l);
                // Cyber_Gear_Motor_Torque(0x00000002, (1) * msg->T_l);
                Cyber_Gear_Motor_Speed(0x00000002, (-1) * msg->dx_r);
                // printf("v_rrrrrrr = %f\n",msg->dx_r);
                Motor_Feedback();
                counter_resent++;
            }
