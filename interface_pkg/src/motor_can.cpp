...
void Interface::Cyber_Gear_Motor_Set_Zero(int ID)
{
    sent_msg[0].ID = (ID & 0xFF) | (0x0600FD << 8);
    sent_msg[0].RemoteFlag = 0; // unchanged
    sent_msg[0].SendType = 1;   // unchanged
    sent_msg[0].ExternFlag = 1; // 扩展帧，因此ID为29位
    sent_msg[0].DataLen = 8;

    sent_msg[0].Data[0] = 0x01; 
    sent_msg[0].Data[1] = 0x00;
    sent_msg[0].Data[2] = 0x00;
    sent_msg[0].Data[3] = 0x00;
    sent_msg[0].Data[4] = 0x00; 
...
void Interface::Cyber_Gear_Motor_Loc(int ID, float output) {
    sent_msg[0].ID = (ID & 0xFF) | (0x1200FD << 8);
    sent_msg[0].RemoteFlag = 0; // unchanged
    sent_msg[0].SendType = 1;   // unchanged
    sent_msg[0].ExternFlag = 1; // 扩展帧，因此ID为29位
    ...
    sent_msg[0].Data[0] = 0x16; // 0x7016是位移命令
    sent_msg[0].Data[1] = 0x70;
    sent_msg[0].Data[2] = 0x00;
    sent_msg[0].Data[3] = 0x00;
    unsigned char* bytePtr = reinterpret_cast<unsigned char*>(&output);
...
double Interface::Cyber_Gear_Motor_GetAndFix_angle_from_PosEncoder(int ID, double & angle_loss_sum_due_to_zeroing, double angle_from_encoder)
{
    //思路：不记录圈数，只记录角度。如果发现某一次已经成功置0，那么angle_loss_sum_due_to_zeroing角度就加上最经一次的angle_from_encoder。
    static std::map<int, double> angle_from_encoder_lasts;
    static std::map<int, bool> is_setting_zeros;

    static int Zero_counter = 0;
    double & angle_from_encoder_last = angle_from_encoder_lasts[ID];
    bool & is_setting_zero = is_setting_zeros[ID];
    double angle_output = 0;


    if(is_setting_zero==true)
    {
        if(angle_from_encoder < -pi || angle_from_encoder > pi)
        {
            angle_output = angle_from_encoder+angle_loss_sum_due_to_zeroing;
            if(++Zero_counter <= 3)
                Cyber_Gear_Motor_Set_Zero(ID);
            else
            {
                ROS_ERROR("Cybergear inconnected!!!");
                ros::shutdown();
            }
        }
        else
        {
            angle_loss_sum_due_to_zeroing += angle_from_encoder_last;
            is_setting_zero = false;
            angle_output = angle_from_encoder+angle_loss_sum_due_to_zeroing;   
        }
    }
    else{
        Zero_counter = 0;
        if(angle_from_encoder >= 2*pi)
        {
            Cyber_Gear_Motor_Set_Zero(ID);
            is_setting_zero = true;
        }
        else if(angle_from_encoder <= -2*pi)
        {
            Cyber_Gear_Motor_Set_Zero(ID);
            is_setting_zero = true;
        }
    ...
    angle_output = angle_from_encoder+angle_loss_sum_due_to_zeroing;
    angle_from_encoder_last = angle_from_encoder;
